...
- [ ] 1.2 Add build/vet/format/test steps to the job: `go build ./...`, `go vet ./...`, a `gofmt -l .` step that fails the job if it prints any file, and `go test -race ./...`. Verify by pushing the branch and confirming the workflow run succeeds on GitHub Actions (all four steps green).
